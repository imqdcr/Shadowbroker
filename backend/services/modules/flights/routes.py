...
def fetch_routes_background(sampled: list) -> None:
    ...
    try:
        now_ts = time.time()
        stale_keys = [k for k, v in dynamic_routes_cache.items()
        ...
        batch_size = 100
        batches = [callsigns_to_query[i:i+batch_size]
                   for i in range(0, len(callsigns_to_query), batch_size)]

        for batch in batches:
            try:
                r = fetch_with_curl("https://api.adsb.lol/api/0/routeset",
                                    method="POST", json_data={"planes": batch}, timeout=15)
                if r.status_code == 200:
                    route_data = r.json()
                    route_list = (route_data.get("value", []) if isinstance(route_data, dict)
                                  else route_data)
