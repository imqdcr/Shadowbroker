...
def get_ais_vessels() -> list[dict]:
    ...
    with _vessels_lock:
        # Prune stale vessels
        stale_keys = [
            k for k, v in _vessels.items() if v.get("_updated", 0) < stale_cutoff
        ]
        for k in stale_keys:
            del _vessels[k]
...
def _ais_stream_loop():
    """Main loop: connect directly to aisstream.io via websocket-client."""
    import websocket
            ...
            while _ws_running:
                ...
                if msg_type in ("PositionReport", "StandardClassBPositionReport"):
                    ...
                    with _vessels_lock:
                        ...
                        vessel["heading"] = (
                            heading if heading != 511 else report.get("Cog", 0)
                        )
                ...
                # Update static data from ShipStaticData
                    ...
                    with _vessels_lock:
                        vessel["name"] = (
                            static.get("Name", "")
                            or metadata.get("ShipName", "UNKNOWN")
                        ).strip() or "UNKNOWN"
